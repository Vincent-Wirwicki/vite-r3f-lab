...
vec2 rotate(in vec2 v, in float r, in vec2 c) {
    ...
    void main(){
      ...
      float noise = terrain(pos.xy *2. )  ;
      
      float dist = length(pos.xy);
      float angle = atan(pos.x, pos.y);
      float force = smoothstep(0.5,0.1,dist);
      float r1 = map(sin(time2 *2. * PI), -1.,1.,0.,.2);
      float logSpiral = log(dist - 0.1) / 2. + angle  ;
      float r2 = map(sin(time *2. * PI) * cos(time *2. * PI), -1.,1.,8.,1.);
      float animSpiral =  sin(time2 * 2. * PI  + logSpiral  * 6.  ) * cos(time2 * 2. * PI  + logSpiral  * 6.  );


      vec2 vrt = vortex(pos.xy )    ;

      vec2 cn = curl(pos.xy   ) *0.1 ;

      
      vec2 flow = vec2(sin(cn.x - vrt.x ) * dist, sin(cn.y - vrt.y  ) *dist   );
      float ease = map(repeat, -1.,1.,0.45,0.65);
      vec2 vort = vortex(pos.xy )  ;
      vec2 velocity = offset.xy   ;
      vec2 nv = normalize(vrt);
      vec3 tt = twist(pos.xyz,4.);
      vec3 grdCirk = sdgCircle(pos.xy, 0.5);

      float c = sdCircle(pos.xy, .35);
      vec2 t = vec2(cos(c - fract(uTime * offset.x)), sin(c  - fract(uTime * offset.y))) * length(pos.xy) ;
      vec2 n = curl(pos.xy);
      velocity *= animSpiral ;
      pos.xy += velocity  ;
  pos.z = normalize(animSpiral )  ;
    // pos.z += mod(animSpiral ,1.)  ;

    // pos.z = length(animSpiral - pos.x)  ;
