...
vec3 rotateX(vec3 position, float angle) {
    ...
    void main(){
        ...
        float camMod = map(sin(time *2. * PI) , -1.,1.,0.,1.);
        ...
        float mn = map(n * 2. * PI , -1.,1. , 0.075 * dist * 2., 0.05 * dist);

        pos.z += mn  ; 
        
        // pos.z -= pow( wave, camMod *0.1);

        // pos.z /= length(waveAmp - wave)*2.15 ;
        //       float offset=l+(angle/(2.*PI))*dis;
        // float circles=mod(offset-iTime,dis);

        float scale = abs(1. - pos.z *10.   ) ; 
        size *=  scale ;
